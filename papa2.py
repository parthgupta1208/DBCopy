...
def everything2():
    ...
    if len(lst1)==1:
        cursor1.execute('select '+str(lst1[0])+' from Source')
        r1=cursor1.fetchall()
        for rec in r1:
            # print('insert into DEST ('+str(lst2[0])+') values '+str(rec).replace(",",""))
            cursor2.execute('insert into DEST ('+str(lst2[0])+') values '+str(rec).replace(",",""))
        ...
        cursor1.execute('select '+(", ".join(lst1))+' from Source')
        r1=cursor1.fetchall()
        for rec in r1:
            # print('insert into DEST ('+(", ".join(lst2))+') values '+str(rec))
            cursor2.execute('insert into DEST ('+(", ".join(lst2))+') values '+str(rec))
        messagebox.showinfo("Succesfully Completed","Records inserted Succesfully !!")
    outdb.commit()
...
l2=Label(text="Destination File").grid(row=2,column=1,sticky="news",padx=10,pady=10)
b1=Button(text="Choose",command=but1)
b1.grid(row=1,column=2,sticky="news",padx=10,pady=10)
b2=Button(text="Choose",command=but2)
b2.grid(row=2,column=2,sticky="news",padx=10,pady=10)
b3=Button(text="Next",command=menu)
b3.grid(row=3,column=1,columnspan=2,sticky="news",padx=10,pady=10)
root.mainloop()